...
export default {
  ...
  methods: {
    ...
    async updateChecked(cart, event) {
      try {
        await this.$store.dispatch("updateChekedById", {
          skuId: cart.skuId,
    ...
    handler: throttle(async function (type, disNum, cart) {
      switch (type) {
        case "add":
          ...
          break;
        case "change":
          disNum = Object.is(Math.abs(parseInt(disNum)), NaN)
            ? 0
            : Math.abs(parseInt(disNum)) - cart.skuNum == -cart.skuNum
            ? -cart.skuNum + 1
            : Math.abs(parseInt(disNum)) - cart.skuNum;
          break;
        case "minus":
